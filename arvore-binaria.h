...
void preInToPost(int in[], int pre[], int numNodes);     // Recebe caminhos pré-ordem e em-ordem e retorna caminho pós-ordem
void deleteTree(NodeBT **root);                          // Deleta a ávore


/* Funções auxiliares */
char equiv(NodeBT *A, NodeBT *B);                        // Conectivo lógico 'bicondicional'
int search(int array[], int v, int size);                // Busca o índice de um elemento em um vetor
